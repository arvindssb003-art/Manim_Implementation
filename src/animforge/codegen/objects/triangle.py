...
class TriangleObjectCodeGenerator(
    ObjectCodeGenerator
):
    """Generate Manim code for Triangle objects."""
    ...
    def generate(
        ...
        if obj.side_length != 2.0:
            lines.append(
                f"{variable_name}.scale("
                f"{obj.side_length / 2}"
                f")"
            )
    ...
    @staticmethod
    def _color_constant(color: str) -> str:
    ...
    def _generate_relative_position(
        ...
        relative = obj.relative_position

        target_variable = variable_map.get(
            relative.target
        )
